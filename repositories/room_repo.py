...
from services.room_codes import normalize_room_number, room_lookup_values
...
class RoomRepository:
    def fetch_rooms(self) -> List[str]:
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT DISTINCT room_number FROM rooms ORDER BY room_number")
            return [normalize_room_number(row[0]) for row in cursor.fetchall() if row[0] is not None]

    def fetch_courses(self, room: str) -> List[str]:
        normalized_room_number, raw_room = room_lookup_values(room)
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
                SELECT DISTINCT co.course_code
                FROM classes c
                ...
                """,
                (normalized_room_number, raw_room),
            )
    ...
    def fetch_sections(self, room: str, course: str) -> List[str]:
        ...
        with get_connection() as conn:
            ...
            cursor.execute(
                ...
                JOIN courses co ON c.course_id = co.course_id
                JOIN rooms r ON c.room_id = r.room_id
                WHERE r.room_number IN (?, ?) AND co.course_code = ?
                ORDER BY c.section
                """,
                (normalized_room_number, raw_room, course),
            )
            return [row[0] for row in cursor.fetchall()]
